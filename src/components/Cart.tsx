...
import { createCheckout } from "../../lib/shopify/shopify";
...
export default function Cart({ isOpen, setIsOpen }: CartProps) {
  ...
  const subtotal = cart.reduce(
    (accumulator, item) =>
      item.price
        ? accumulator + parseFloat(item.price.slice(1)) * item.quantity
        : accumulator,
    0
  );
  ...
  async function handleCheckout(event: SyntheticEvent) {
    ...
    const { checkout, errors } = await createCheckout(lineItems);
